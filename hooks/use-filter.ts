...
export function useFilter<T = Song>({
  ...
  const filteredItems = useMemo(() => {
    ...
    const filter = filterFn || (defaultSongFilter as (item: T, filters: Record<string, string>, searchQuery: string) => boolean)
    const sort = sortFn || (defaultSongSort as (items: T[], sortOption: SortOption) => T[])
